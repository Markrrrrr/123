...
{
  //GPIO_Init(GPIOB, GPIO_Pin_4, GPIO_Mode_Out_PP_Low_Fast);//设置SD_CS对应的DSP引脚为输出
  PB_DDR_bit.DDR4=1;PB_CR1_bit.C14=1;PB_CR2_bit.C24=1;//RST
  SPI1_Init();
  SD_CS_H;
}
///////////////////////////////////////////////////////////////////////////////////
...
//len:要读取的数据长度.
//返回值:0,成功;其他,失败;	
unsigned char SD_RecvData(unsigned char*buf,u16 len)
{		  	  
...
//向SD卡发送一个命令
//输入: unsigned char cmd   命令 
//      u32 arg  命令参数
//      unsigned char crc   crc校验值	   
//返回值:SD卡返回的响应															  
unsigned char SD_SendCmd(unsigned char cmd, u32 arg, unsigned char crc)
{
  unsigned char r1;	
...
//获取SD卡的CID信息，包括制造商信息
//输入: unsigned char *cid_data(存放CID的内存，至少16Byte）	  
//返回值:0：NO_ERR
//		 1：错误														   
